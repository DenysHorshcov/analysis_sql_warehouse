-- створити схему на випадок, якщо її ще немає
...
BEGIN
  ...
  secs := EXTRACT(EPOCH FROM (end_time - start_time));
  RAISE NOTICE '>> Load Duration: % seconds', secs;
  RAISE NOTICE '>> -------------';
...
SELECT
    ...
    COALESCE(
        (LEAD(prd_start_dt) OVER (PARTITION BY prd_key 
            ...
            - INTERVAL '1 day')::date,
        DATE '9999-12-31'
    )                                                                       AS prd_end_dt     -- End date = next start - 1 day
FROM bronze.crm_prd_info;
  ...
  SELECT
    CASE
      WHEN cid LIKE 'NAS%' THEN SUBSTRING(cid FROM 4)  -- прибираємо 'NAS'
      ELSE cid
    END AS cid,
    CASE
  ...
  secs := EXTRACT(EPOCH FROM (batch_end_time - batch_start_time));
  RAISE NOTICE '==========================================';
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    BEGIN
      ...
      RAISE NOTICE 'SQLSTATE : %', err_state;
      ...
      RAISE NOTICE '==========================================';
      RAISE; -- щоб CALL завершився помилкою
    END;
END;
$$;

-- Виклик:
-- CALL silver.load_silver(); та сама проблема чому?
